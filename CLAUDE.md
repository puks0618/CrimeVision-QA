...
#### 1. Reasoner — Llama-3.1-8B QLoRA (answer generation)
```bash
python finetune/generate_training_data.py           # Build instruction-response pairs from MongoDB
python finetune/train_qlora.py                      # QLoRA on Llama-3.1-8B-Instruct (~30 min on A100)
python finetune/eval_finetuned.py --adapter-path finetune/output/checkpoint-final --video-id <id> --training-data finetune/data/training_data.json
```

#### 2. Frame Describer — Qwen2-VL-7B QLoRA (replaces kimi-k2p5 API)
```bash
python finetune/generate_frame_data.py              # Pull frame images + existing descriptions from MongoDB
python finetune/train_frame_describer.py            # QLoRA on Qwen2-VL-7B-Instruct (~2 hrs on A100)
# Set LOCAL_VLM_ADAPTER_PATH=finetune/output/frame_describer/checkpoint-final in .env
```

#### 3. Whisper — whisper-medium (replaces Fireworks Whisper-v3 API)
```bash
python finetune/generate_whisper_data.py            # Extract audio clips + transcripts from MongoDB
python finetune/train_whisper.py                    # Fine-tune whisper-medium (~25 min on A100)
# Set LOCAL_WHISPER_PATH=finetune/output/whisper/checkpoint-final in .env
```

#### 4. Embeddings — BGE-base-en (replaces Fireworks GTE-large API, same 768-dim)
```bash
python finetune/generate_embedding_data.py          # Build (query, positive, negative) triplets
python finetune/train_embeddings.py                 # TripletLoss on BGE-base-en-v1.5 (~15 min on A100)
# Set EMBED_PROVIDER=local and LOCAL_EMBED_PATH=finetune/output/embeddings/checkpoint-final in .env
```
