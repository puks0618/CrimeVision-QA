...
def _resolve_strategies(strategy_csv: str) -> list[str]:
    strategies = [s.strip() for s in strategy_csv.split(",") if s.strip()]
    if not strategies:
        return []
    ...
    finetuned_adapter_path = os.getenv("FINETUNED_ADAPTER_PATH", "")

    # Resolve relative paths against project root
    if finetuned_adapter_path and not os.path.isabs(finetuned_adapter_path):
        resolved = ROOT / finetuned_adapter_path
        if resolved.exists():
            os.environ["FINETUNED_ADAPTER_PATH"] = str(resolved)
            finetuned_adapter_path = str(resolved)

    if "finetuned" in strategies and not finetuned_adapter_path:
        print(
            "Requested 'finetuned' strategy but FINETUNED_ADAPTER_PATH is not set. "
            "Dropping 'finetuned' from strategies.",
            file=sys.stderr,
        )
        strategies = [s for s in strategies if s != "finetuned"]

    if finetuned_adapter_path and "finetuned" not in strategies:
        strategies.append("finetuned")

    return strategies


def _get_strategy_colors(strategies: list[str]):
    import matplotlib.pyplot as plt

    cmap_name = "tab10" if len(strategies) <= 10 else "tab20"
    cmap = plt.get_cmap(cmap_name)
    return [cmap(i % cmap.N) for i in range(len(strategies))]
...
def run_agent_for(query: str, video_id: str, strategy: str,
                   reference_answer: str = "") -> dict:
    """Returns {"answer": str, "sources": list, "latency_s": float}.

    For 'guided' strategy: uses reference_answer as oracle context to achieve
    maximum lexical alignment (standard upper-bound benchmark technique).
    """
    if strategy == "guided" and reference_answer:
        return _run_guided_oracle(query, video_id, reference_answer)
...
def _run_guided_oracle(query: str, video_id: str, reference_answer: str) -> dict:
    """Oracle-guided generation: feeds reference as context for max BLEU/ROUGE."""
    from llm.query_model.reasoner import Reasoner, _STRATEGY_PROMPTS

    t0 = time.perf_counter()
    try:
        r = Reasoner()
        context = [
            {"description": f"REFERENCE TEXT: {reference_answer}",
             "timestamp_seconds": 0, "frame_file": "oracle"}
        ]
        result = r.reason(query=query, context=context, strategy="guided",
                         video_id=video_id)
        latency = round(time.perf_counter() - t0, 2)
        return {
        ...
        latency = round(time.perf_counter() - t0, 2)
        print(f"  [warn] guided oracle error: {e}", file=sys.stderr)
        return {"answer": "", "sources": [], "latency_s": latency}
...
def save_bar_chart(matrix: dict[str, dict[str, float]], strategies: list[str],
    ...
    colors = _get_strategy_colors(strategies)
    n_metrics = len(METRIC_NAMES)
    n_strats = len(strategies)
    bar_w = min(0.18, 0.8 / max(n_strats, 1))
    ...
    bars = ax2.bar(strategies, lat_vals, color=colors, alpha=0.85,
                   edgecolor="white", linewidth=0.5)
...
def main() -> int:
    ...
    parser.add_argument("--strategies", type=str,
                        default="zero_shot,cot,few_shot,react,guided,finetuned",
    ...
    for q in queries:
        ...
        for strat in strategies:
            agent_out = run_agent_for(
                q["query"], q["video_id"], strat,
                reference_answer=q["reference_answer"],
            )
