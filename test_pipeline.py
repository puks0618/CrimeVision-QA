...
def _find_ffmpeg() -> str:
    """Locate ffmpeg binary — checks PATH first, then common local installs."""
    import shutil
...
def extract_audio(video_path: str, audio_path: str) -> str | None:
    ...
    result = subprocess.run([ffmpeg_bin, "-version"], capture_output=True)
    if result.returncode != 0:
        print(f"  [Audio] ffmpeg not found at '{ffmpeg_bin}'. Install with: brew install ffmpeg")
        return None

    os.makedirs(os.path.dirname(os.path.abspath(audio_path)), exist_ok=True)
    cmd = [ffmpeg_bin, "-i", video_path, "-vn", "-acodec", "libmp3lame", "-q:a", "2", "-y", audio_path]
